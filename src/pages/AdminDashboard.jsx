...
export default function AdminDashboard() {
  ...
  // EXCLUSIVE BULK ARCHIVE UPLOADER (DREAM CREATIONS)
  const handleBulkArchiveUpload = async (e) => {
    if (!bulkCategory || !bulkSubtitle) {
      alert("Please select a Category and Subtitle first!");
      return;
    }
    ...
    alert(`🚀 Initializing bulk deployment pipeline for ${files.length} archive project(s)...`);
    let successCounter = 0;
    let failureCounter = 0;
    const newArchiveProjects = [];

    for (const file of files) {
      try {
        const fileExt = file.name.split('.').pop();
        
        // AUTO-EXTRACT: Clean raw name for Title
        const rawName = file.name.replace(`.${fileExt}`, '');
        const titleFromFileName = rawName.replace(/[-_]/g, ' '); // Pwede ding hindi i-replace, pero mas malinis to.
        ...
        console.error(`Project archive abort block inside: ${file.name}`, err);
        failureCounter++;
      }
    }

    if (newArchiveProjects.length > 0) {
      setDreamArchive(prev => [...newArchiveProjects, ...prev]);
    }

    alert(`🟢 BULK ARCHIVE PROCESSED!\nProjects Added: ${successCounter}\nFailed: ${failureCounter}\n\nDon't forget to click "SAVE MODULE" to push to Cloud.`);
  };
            ...
            <div className="space-y-8 text-left">
              ...
              <div className="p-6 rounded-2xl border border-zinc-900 bg-zinc-950/40 space-y-4">
                <div className="flex justify-between items-center border-b border-zinc-900 pb-2 flex-wrap gap-2">
                  <h4 className="text-xs font-mono font-bold text-zinc-400 uppercase tracking-widest w-full xl:w-auto">// Project Archive Matrix Registries</h4>
                  <div className="flex flex-wrap items-center gap-2 w-full xl:w-auto">
                    <select value={bulkCategory} onChange={(e) => { setBulkCategory(e.target.value); setBulkSubtitle(''); }} className="bg-zinc-950 border border-zinc-900 rounded-lg p-1.5 text-[10px] text-zinc-400 font-bold outline-none cursor-pointer">
                      <option value="" disabled>Select Bulk Category...</option>
                      {creationsCategories.map(c => <option key={c.id} value={c.category}>{c.category}</option>)}
                    </select>
                    <select value={bulkSubtitle} onChange={(e) => setBulkSubtitle(e.target.value)} className="bg-zinc-950 border border-zinc-900 rounded-lg p-1.5 text-[10px] text-zinc-400 outline-none cursor-pointer" disabled={!bulkCategory}>
                      <option value="" disabled>Select Bulk Subtitle...</option>
                      {bulkCategory && creationsCategories.find(c => c.category === bulkCategory)?.items.map(sub => <option key={sub} value={sub}>{sub}</option>)}
                    </select>
                    <div className="relative">
                      <input type="file" multiple onChange={handleBulkArchiveUpload} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" accept="image/*,video/mp4" disabled={!bulkCategory || !bulkSubtitle} />
                      <button disabled={!bulkCategory || !bulkSubtitle} className="px-2.5 py-1 text-[10px] font-mono bg-zinc-800 border border-zinc-700 rounded-lg text-white font-bold flex items-center gap-1 hover:bg-zinc-700 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed">
