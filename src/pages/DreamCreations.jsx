...
const isVideo = (url) => {
  if (!url) return false;
  // Enhanced to ensure it doesn't treat YouTube links as native videos
            ...
            <div key={cat.id} className="pt-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                {cat.items.map((subtitle, idx) => {
                  ...
                  // YOUTUBE AUTO-THUMBNAIL LOGIC FOR COVER
                  const ytID = getYouTubeID(rawCover);
                  const finalCover = ytID ? `https://img.youtube.com/vi/${ytID}/maxresdefault.jpg` : rawCover;
                  const isYt = !!ytID;
                  ...
                  return (
                    <button key={idx} id={subtitle.toLowerCase().replace(/\s+/g, '-')} onClick={() => openPortfolioGallery(subtitle)} className="relative h-48 rounded-2xl overflow-hidden group cursor-pointer border border-white/10 text-left transition-all duration-500">
                      {!isYt && isVideo(finalCover) ? (
                        <video key={finalCover} src={`${finalCover}#t=0.1`} className="absolute inset-0 w-full h-full object-cover opacity-60 group-hover:scale-110 transition-transform duration-700 pointer-events-none" autoPlay loop muted playsInline preload="metadata" onError={(e) => { e.target.style.display = 'none'; e.target.nextSibling.style.display = 'block'; }} />
                      ) : (
                        <img key={finalCover} src={finalCover} alt={subtitle} className="absolute inset-0 w-full h-full object-cover opacity-60 group-hover:scale-110 transition-transform duration-700 pointer-events-none" onError={(e) => { e.target.style.display = 'none'; e.target.nextSibling.style.display = 'block'; }} />
                      )}
                ...
                <div className="columns-2 sm:columns-3 lg:columns-4 gap-0.5 space-y-0.5 block">
                  {visualProjects.length > 0 ? (
                    visualProjects.map((project) => {
                      // YOUTUBE AUTO-THUMBNAIL LOGIC FOR INNER MASONRY GRID
                      const pYtID = getYouTubeID(project.featured_image_url) || getYouTubeID(project.video_url);
                      const pDisplayImg = pYtID ? `https://img.youtube.com/vi/${pYtID}/maxresdefault.jpg` : project.featured_image_url;
                        ...
                        >
                          {pDisplayImg ? ( 
                            !pYtID && isVideo(pDisplayImg) ? (
                              <video src={`${pDisplayImg}#t=0.1`} className="w-full h-auto block object-cover group-hover:scale-105 transition-transform duration-500 pointer-events-none" loop muted playsInline preload="metadata" />
                            ) : (
                              <>
                                <img src={pDisplayImg} alt={project.title} className="w-full h-auto block object-cover group-hover:scale-105 transition-transform duration-500 pointer-events-none" /> 
                                {pYtID && (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {filteredProjects.length > 0 ? (
                    filteredProjects.map((project) => {
                      ...
                      const bpDisplayImg = bpYtID ? `https://img.youtube.com/vi/${bpYtID}/maxresdefault.jpg` : project.featured_image_url;
                           ...
                           <div className="aspect-video relative overflow-hidden bg-black/60">
                             {bpDisplayImg ? ( 
                               !bpYtID && isVideo(bpDisplayImg) ? (
                                 <video src={`${bpDisplayImg}#t=0.1`} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500 pointer-events-none" autoPlay loop muted playsInline preload="metadata" />
                               ) : (
                                 <img src={bpDisplayImg} alt={project.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500 pointer-events-none" /> 
                               )
