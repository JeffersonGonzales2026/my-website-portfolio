...
// We separated the number from the "+" so the code knows how to count mathematically
const statsData = [
...
// This sub-component handles the "Computing" number scrolling animation
function Counter({ num, suffix }) {
  const nodeRef = useRef(null);
  // Detects when the number scrolls into the user's screen
  ...
  useEffect(() => {
    if (isInView) {
      ...
      const controls = animate(0, num, {
        ...
        onUpdate(value) {
          if (nodeRef.current) {
            // Updates the text instantly without triggering heavy React re-renders
